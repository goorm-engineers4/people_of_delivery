...
public interface ReviewRepository extends JpaRepository<Review, UUID> {
    @Query("select r from Review r where r.isDeleted = false and r.user.id =:UserId and r.user.isDeleted = false")
    List<Review> findAllByUserId(@Param("UserId") UUID userId);

    @Query("select r from Review r where r.isDeleted = false and r.store.id =:StoreId and r.store.isDeleted = false")
    List<Review> findAllByStoreId(@Param("StoreId") UUID storeId);
