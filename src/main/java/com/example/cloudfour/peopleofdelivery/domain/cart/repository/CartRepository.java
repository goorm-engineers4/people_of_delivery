...
public interface CartRepository extends JpaRepository<Cart, UUID> {
    @Query("select c from Cart c where c.id = :cartId and c.user.id = :userId and c.user.deleted = false")
    Optional<Cart> findByIdAndUser(@Param("cartId") UUID cartId, @Param("userId") UUID userId);

    @Query("select count(c) > 0 from Cart c where c.user.id = :userId and c.store.id = :storeId and c.user.deleted = false")
